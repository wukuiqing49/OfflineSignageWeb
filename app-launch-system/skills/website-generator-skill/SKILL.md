...
9. Confirm metadata, canonical and alternate locale links, Open Graph data, JSON-LD, `robots.txt`, `sitemap.xml`, the web manifest, Bing Webmaster verification, and IndexNow key output use confirmed values only. The generator deliberately omits canonical and Open Graph URL tags when `websiteUrl` is unknown.
...
- Use semantic HTML, visible focus states for links and buttons, descriptive alt text, and reduced-motion support. The language selector intentionally has no focus outline.
- Avoid decorative gradients, nested cards, oversized empty hero areas, and visuals that obscure the app.

## Localization

Generator-owned labels for overview, feature pages, blogs, video, and company pages must resolve through the locale map; a non-source locale must never fall back to English UI text.
...
For every page, generate `LOCALE_ROUTES_JSON` and a native-language `<select data-locale-switcher>` using only locales where that page exists. The recommended international website set is `en-US`, `zh-CN`, `ja-JP`, `ko-KR`, `es-ES`, `fr-FR`, `de-DE`, and `pt-BR`; add other BCP 47 locales only with a prepared translation file. Each configured target must have a complete native-language translation file, including navigation, homepage, support, privacy, and company labels. Company identity is shared across locales: keep the legal name, developer name, website, email, and address unchanged, while translating the company description and labels in `organization.yaml.localized`. Enable automatic detection only on source-locale root pages. Match a saved explicit choice first, then `navigator.languages`, configured aliases, base language, and finally the source locale. Store explicit selection locally, never overwrite it with automatic detection, and never redirect localized subdirectory pages automatically. Mark every non-reviewed locale as `machine-draft` in `<project-root>/localization-status.yaml`.
